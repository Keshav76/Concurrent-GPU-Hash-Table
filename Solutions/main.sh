...
for i in "${!trace_groups[@]}"; do
    ...
    for label in "${!config_map[@]}"; do
        ...
        lkp_val=$(echo "scale=2; $lkp_raw / 100" | bc)
        rem_val=$(echo "scale=2; $rem_raw / 100" | bc)

        ./script.sh --size="$size" --ins="$ins_val" --lkp="$lkp_val" --rem="$rem_val" \
                 --insert_path="$insert_file" --delete_path="$delete_file" --search_path="$search_file"

        # Move output files
        for f in "output_Better GPU.txt" "output_SlabHash.txt"; do
            if [ -f "$f" ]; then
                mv "$f" "$output_dir/"
            else
