...
class cuckoo_t{
    public:
        ...
        // --- Add these lines for temp slots ---
        key_t* temp_key_slot[table_num];         // 1 key slot per table
        value_t* temp_value_slot[table_num];     // 1 value slot per table
        uint32_t* temp_slot_occupied[table_num]; // 1 flag per table (optional)
        // --------------------------------------
        ...
        static void device_table_mem_init(cuckoo_t &mycuckoo, uint32_t single_table_size){
            for(uint32_t i = 0; i < table_num; i++){
                ...
                // --- Allocate temp slot memory and initialize ---
                cnmemMalloc((void**)&(mycuckoo.temp_key_slot[i]), sizeof(key_t), 0);
                cnmemMalloc((void**)&(mycuckoo.temp_value_slot[i]), sizeof(value_t), 0);
                cnmemMalloc((void**)&(mycuckoo.temp_slot_occupied[i]), sizeof(uint32_t), 0);
                cudaMemset(mycuckoo.temp_key_slot[i], 0, sizeof(key_t));
                cudaMemset(mycuckoo.temp_value_slot[i], 0, sizeof(value_t));
                cudaMemset(mycuckoo.temp_slot_occupied[i], 0, sizeof(uint32_t));
                // ----------------------------------------------
            }
