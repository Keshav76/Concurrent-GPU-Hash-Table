...
namespace DynamicHash{
    ...
    DEVICEQUALIFIER
    void flush_temp_slots(thread_block_tile<cg_size> group) {
        for (uint32_t table_no = 0; table_no < DataLayout<>::table_num; ++table_no) {
            if (*(cuckoo_table.temp_slot_occupied[table_no])) {
                key_t temp_key = *(cuckoo_table.temp_key_slot[table_no]);
                value_t temp_value = *(cuckoo_table.temp_value_slot[table_no]);
                uint32_t table_len = cuckoo_table.table_size[table_no];
                uint32_t hash_val = ch::caculate_hash((uint32_t)temp_key, table_no, table_len);
                key_bucket_t* key_insert_bucket = cuckoo_table.key_table_group[table_no] + hash_val;
                value_bucket_t* value_insert_bucket = cuckoo_table.value_table_group[table_no] + hash_val;

                // Try to find an empty slot
                bool inserted = false;
                for (uint32_t i = 0; i < bucket_size; ++i) {
                    if (key_insert_bucket->bucket_data[i] == empty_key) {
                        key_insert_bucket->bucket_data[i] = temp_key;
                        value_insert_bucket->bucket_data[i] = temp_value;
                        inserted = true;
                        break;
                    }
                }
                if (inserted) {
                    *(cuckoo_table.temp_slot_occupied[table_no]) = 0;
                }
            }
        }
    }
    ...
    void cg_insert_with_lock(key_t &key, value_t value, uint32_t pre_table_no, bool active, thread_block_tile<cg_size> group){
        ...
        while(group.any(active == true)){
            ...
            if(group_mask2 != 0){
                ...
                //evict
                if(group.thread_rank() == leader){
                    key_t displaced_key = (key_insert_bucket->bucket_data)[leader];
                    value_t displaced_value = (value_bucket->bucket_data)[leader];
                    (key_insert_bucket->bucket_data)[leader] = insert_key;
                    (value_bucket->bucket_data)[leader] = smem.value_to_write[threadIdx.x / CgSize];
                    //unlock
                    atomicExch(bucket_lock_addr, unlock_tag);
                
                    // Compute next table index for temp slot:
                    uint32_t next_pair = ch::get_pair((uint32_t)displaced_key);
                    uint32_t next_table_no = ch::get_table1_no(next_pair);
                    if(next_table_no == insert_table_no){
                        next_table_no = ch::get_table2_no(next_pair);
                    }
                
                    // Store in temp slot:
                    *(cuckoo_table.temp_key_slot[next_table_no]) = displaced_key;
                    *(cuckoo_table.temp_value_slot[next_table_no]) = displaced_value;
                    *(cuckoo_table.temp_slot_occupied[next_table_no]) = 1;
                
                    // Stop further displacement for this insert
                    active = false;
                }
    ...
    void cuckoo_insert(key_t *keys, value_t *values, uint32_t data_num){
        ...
        for(; tid < data_num ; tid += step){
            flush_temp_slots(group); // <-- NEW: flush temp slots before each insert
    ...
    void cuckoo_search(key_t* keys, value_t* values, uint32_t size){
        ...
        for(;group_index_in_all < size; group_index_in_all += step){
            ...
            if (!flag) {
                // Check temp slots
                for (uint32_t t = 0; t < DataLayout<>::table_num; ++t) {
                    if (*(cuckoo_table.temp_slot_occupied[t]) &&
                        *(cuckoo_table.temp_key_slot[t]) == key) {
                        values[group_index_in_all] = *(cuckoo_table.temp_value_slot[t]);
                        flag = true;
                        break;
                    }
                }
        ...
        // After normal bucket search, add:
    }
    ...
    void cuckoo_delete(key_t* keys, value_t* values, uint32_t size){
        ...
        for(;group_index_in_all < size; group_index_in_all += step){
            ...
            // After normal bucket delete, add:
            if (!flag) {
                // Check and clear temp slots
                for (uint32_t t = 0; t < DataLayout<>::table_num; ++t) {
                    if (*(cuckoo_table.temp_slot_occupied[t]) &&
                        *(cuckoo_table.temp_key_slot[t]) == key) {
                        *(cuckoo_table.temp_slot_occupied[t]) = 0;
                        break;
                    }
                }
            }
